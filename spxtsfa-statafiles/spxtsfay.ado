...
capture program drop spxtsfay
program define spxtsfay, eclass sortpreserve
version 16

	if replay() {
		if (`"`e(cmd)'"' != "spxtsfay") error 301
		Replay `0'
	}
	else	Estimate `0'
end
...
syntax varlist,Uhet(varlist) [INItial(name) NOCONstant NORMalize(string) ///
							  ...
							  MLSEarch(string) MLMAXopt(string) DELVE CONSTraints(string) wy(string) ///
							  lndetfull lndetmc(numlist >0 min=2 max=2) wx(string) NOLOG] 
...
//mata mata describe
	ml model d0 spxtsfay`bp'() (frontier:`yvar' = `xvars' `wxvars2',`noconstant')(uhet: `uhet',noconstant) ///
	                         /lnsigma2_u  /lnsigma2_v (Wy:), nopreserve `cns' `mlmodelopt' title(`title')
   ...
   if(`"`te'"'!=""){
		...
		mata:_te_order=spxtsfay_te(_b_ml,`nx',`nz',"`yvar'","`xvars' `wxvars2'","`uhet'","`noconstant'")
   }	
