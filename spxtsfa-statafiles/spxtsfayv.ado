...
capture program drop spxtsfayv
program define spxtsfayv, eclass sortpreserve
version 16

	if replay() {
		if (`"`e(cmd)'"' != "xtsfayv0") error 301
		Replay `0'
	}
	else	Estimate `0'
end
...
syntax syntax varlist(fv ts min=2),  Uhet(varlist fv ts) wv(string) wy(string) [INItial(name) NOCONstant ///
...
//mata mata describe
	ml model d0 spxtsfayv`bp'() (frontier:`yvar' = `xvars' `wxvars2',`noconstant')(uhet: `uhet',noconstant) ///
	                         /lnsigma2_u  /lnsigma2_v (Wy:) (Wv:), nopreserve `cns' `mlmodelopt' title(`title')
   ...
   Replay , `diopts' 
    ...
    if(`"`te'"'!=""){
		...
		mata:_te_order=spxtsfayv_te(_b_ml,`nx',`nz',"`yvar'","`xvars' `wxvars2'","`uhet'","`noconstant'")
   }	
