...
capture program drop spxtsfayuv0
program define spxtsfayuv0, eclass sortpreserve
version 16

	if replay() {
		if (`"`e(cmd)'"' != "spxtsfayuv0") error 301
		Replay `0'
	}
	else	Estimate `0'
end
...
**************
   * generating Wx
	if(`"`wxvars'"'!=""){
      qui genwxvars `wxvars', aname(w_ina) tvar(`time2')
...
//mata mata describe
	ml model d0 spxtsfayuv0`bp'() (frontier:`yvar' = `xvars' `wxvars2',`noconstant')(uhet: `uhet',noconstant) ///
   ...
   if(`"`te'"'!=""){
		...
		mata:_te_order=spxtsfayuv0_te(_b_ml,`nx',`nz',"`yvar'","`xvars' `wxvars2'","`uhet'","`noconstant'")
   }	
