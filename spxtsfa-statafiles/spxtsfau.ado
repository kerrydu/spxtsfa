...
capture program drop spxtsfau
program define spxtsfau, eclass sortpreserve
version 16

	if replay() {
		if (`"`e(cmd)'"' != "spxtsfau") error 301
		Replay `0'
	}
	else	Estimate `0'
end
...
************
	...
	ml model d0 spxtsfau() (frontier:`yvar' = `xvars' `wxvars2',`noconstant')(uhet: `uhet',noconstant) ///
	                         /lnsigma2_u  /lnsigma2_v  (Wu:), nopreserve `cns' `mlmodelopt' title(`title')
   ...
   if(`"`te'"'!=""){
		...
		*/
		...
		mata:_te_order=spxtsfau_te(_b_ml,`nx',`nz',"`yvar'","`xvars' `wxvars2'","`uhet'","`noconstant'")
   }	
