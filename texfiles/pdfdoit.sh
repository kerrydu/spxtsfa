#!/bin/sh
# doit.sh
# version 1.0.0  01sep2022
# Written by:   Jeff Pitblado
# Description:  shell script to run -lualatex- on a stata journal insert
...
RUNTEX () {
	if ! lualatex --interaction batchmode $MAIN
	then
		echo
		echo "$self: error occured with (lualatex $MAIN)"
...
echo "$self:  ready for viewing: $MAIN.pdf"
